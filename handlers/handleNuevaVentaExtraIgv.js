...
export const handleNuevaVentaExtraIgv = async (req, res) => {
    try {
        const body = await req.body;
        console.log("entre a handleNuevaVentaExtraIgv", body)
        ...
        if (nombreProducto === undefined || nombreProducto === null || nombreProducto === "" ||
            ...
            if (prevStatus == "0") {
                ...
                const acctions = await VentaExtraIgv.findByIdAndUpdate(body.id, { $set: { status: status, medioPago: medioPago } }).exec();
                ...
                const addNewVentaExtraIgv = await VentaExtraIgv.create(body);
                res.status(201).json({
                    message: 'Estos son los datos de la nueva venta bazar',
                    data: addNewVentaExtraIgv, // Datos del usuario decodificados del token
                    status: 201,
                });
            }
        }
        ...
        console.error("❌ Error en handleNuevaVentaExtraIgv:", error);
