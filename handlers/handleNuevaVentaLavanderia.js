...
export const handleNuevaVentaLavanderia = async (req, res) => {
    try {
        const body = await req.body;
        console.log("entre a handleNuevaVentaLavanderia", body)
        ...
        if (nombreProducto === undefined || nombreProducto === null || nombreProducto === "" ||
            ...
            if (prevStatus == "0") {
                ...
                const acctions = await VentaLavanderia.findByIdAndUpdate(body.id, { $set: { status: status, medioPago: medioPago } }).exec();
                ...
                const addNewVentaLavanderia = await VentaLavanderia.create(body);
                res.status(201).json({
                    message: 'Estos son los datos de la nueva venta bazar',
                    data: addNewVentaLavanderia, // Datos del usuario decodificados del token
                    status: 201,
                });
            }
        }
        ...
        console.error("❌ Error en handleNuevaVentaLavanderia:", error);
