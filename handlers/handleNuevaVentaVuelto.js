...
export const handleNuevaVentaVuelto = async (req, res) => {
    try {
        const body = await req.body;
        console.log("entre a handleNuevaVentaVuelto", body)
        ...
        if (
            ...
            if (prevStatus == "0") {
                ...
                const acctions = await VentaVuelto.findByIdAndUpdate(body.id, { $set: { status: status, medioPago: medioPago, comentario: comentario } }).exec();
                ...
                const addNewVentaVuelto = await VentaVuelto.create(body);
                res.status(201).json({
                    message: 'Estos son los datos de la nueva venta bazar',
                    data: addNewVentaVuelto, // Datos del usuario decodificados del token
                    status: 201,
                });
            }
        }
        ...
        console.error("❌ Error en handleNuevaVentaVuelto:", error);
